...
def _rewrite_localization_params(
    ...
    fd, rewritten_path = tempfile.mkstemp(
        prefix="machida_lidar_localization_", suffix=".yaml"
    )
    os.close(fd)
...
def _start_lidar_localization(context, *args, **kwargs):
    ...
    initial_pose_values = {
        ...
        "initial_pose_qx": float(
            LaunchConfiguration("initial_pose_qx").perform(context)
        ),
        "initial_pose_qy": float(
            LaunchConfiguration("initial_pose_qy").perform(context)
        ),
        "initial_pose_qz": float(
            LaunchConfiguration("initial_pose_qz").perform(context)
        ),
        "initial_pose_qw": float(
            LaunchConfiguration("initial_pose_qw").perform(context)
        ),
    }

    if _as_optional_bool(
        LaunchConfiguration("use_odom_localization_demo").perform(context)
    ):
        return [
            LogInfo(
                msg=(
    ...
    localization_param_dir = LaunchConfiguration("localization_param_dir").perform(
        context
    )
    pcd_map_path = LaunchConfiguration("pcd_map_path").perform(context).strip()
    set_initial_pose = _as_optional_bool(
    ...
    actions.append(
        IncludeLaunchDescription(
            ...
            launch_arguments={
                ...
                "publish_lidar_tf": LaunchConfiguration("publish_lidar_tf").perform(
                    context
                ),
                "lidar_frame_id": LaunchConfiguration("lidar_frame_id").perform(
                    context
                ),
                ...
                "lidar_tf_pitch": LaunchConfiguration("lidar_tf_pitch").perform(
                    context
                ),
                "lidar_tf_yaw": LaunchConfiguration("lidar_tf_yaw").perform(context),
                "publish_imu_tf": LaunchConfiguration("publish_imu_tf").perform(
                    context
                ),
...
def _start_map_server(context, *args, **kwargs):
    launch_map_server = _as_optional_bool(
        LaunchConfiguration("launch_map_server").perform(context)
    )

    if not launch_map_server:
        return [
            LogInfo(
                msg="launch_map_server is false. Skipping map_server and its lifecycle manager."
            )
        ]
...
def generate_launch_description():
    ...
    declarations = [
        ...
        DeclareLaunchArgument(
            "reinitialization_supervisor_use_latest_pose", default_value="false"
        ),
        DeclareLaunchArgument(
            "reinitialization_supervisor_publish_count", default_value="1"
        ),
        DeclareLaunchArgument(
            "reinitialization_supervisor_publish_interval_sec",
            default_value="0.2",
        ...
        DeclareLaunchArgument(
            "localizer_enable_timer_publishing", default_value="true"
        ),
        ...
        DeclareLaunchArgument("realsense_max_obstacle_height", default_value="2.0"),
        ...
        DeclareLaunchArgument(
            "use_head_camera",
            default_value="true",
            description="Tilt the head camera with the head servo during navigation.",
        ),
        DeclareLaunchArgument(
            "augmented_costmap_topic", default_value="/augmented_local_costmap"
        ),
        ...
        DeclareLaunchArgument("max_linear_velocity", default_value="1.0"),
        ...
        parameters=[
            {
                ...
                "realsense_min_obstacle_height": LaunchConfiguration(
                    "realsense_min_obstacle_height"
                ),
                "realsense_max_obstacle_height": LaunchConfiguration(
                    "realsense_max_obstacle_height"
                ),
                "realsense_min_sensor_range": LaunchConfiguration(
        ...
        parameters=[
            {
                ...
                "planner_obstacle_threshold": LaunchConfiguration(
        ...
        parameters=[
            {
                ...
                "path_obstacle_threshold": LaunchConfiguration(
                ...
                "augmented_costmap_topic": LaunchConfiguration(
