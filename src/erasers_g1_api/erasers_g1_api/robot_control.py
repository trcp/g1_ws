...
class G1Navigation():
    ...
    def _precision_correction(self, goal_pose: PoseStamped, pos_tol: float, yaw_tol: float):
        ...
        self.__node.get_logger().info("Starting precision correction using TF and /imu...")
        ...
        with TemporarySubscriber(self.__node, Imu, '/imu', 10, imu_cb):
            ...
            while rclpy.ok() and time.time() - start_time < 5.0:
                ...
                if dist <= pos_tol and abs(eyaw) <= yaw_tol:
                    self.__node.get_logger().info(f"Precision correction completed. Dist: {dist:.3f}, YawErr: {eyaw:.3f}")
                    break
                ...
                vx = apply_min_max(lex, 2.0, 0.2, 0.5, pos_tol/2.0)
                vy = apply_min_max(ley, 2.0, 0.2, 0.5, pos_tol/2.0)
